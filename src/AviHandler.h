...
to only use the avi header code. Adds AviWriter class to encapsulate
writing from the espressif frame2jpg function.
...
int maxFrames = 20000;
#define doRestart(_msg) {M5_LOGE(_msg); delay(50000); abort(); }
...
public:
  bool begin(fs::FS &fs, const char *path, uint8_t frameType, uint8_t targetFps)
  {
    ...
    _file.write(aviHeader, AVI_HEADER_LEN);
  ...
  bool addJpegFrame(const uint8_t *jpg, size_t jpgLen)
  {
    ...
    // pad to 4-byte boundary (same as project)
    uint16_t filler = (4 - (jpgLen & 0x3)) & 0x3;
    uint32_t paddedSize = (uint32_t)jpgLen + filler;

    uint8_t hdr[CHUNK_HDR];
    memcpy(hdr, dcBuf, 4);
    memcpy(hdr + 4, &paddedSize, 4);

    // write chunk header + jpeg data + padding
    _file.write(hdr, CHUNK_HDR);
    _file.write(jpg, jpgLen);
    while (filler--)
      _file.write((uint8_t)0x00);

    // add index entry for this chunk
    buildAviIdx(paddedSize, true, false);
  ...
  bool end()
  {
    ...
    // finalize index + append it
    finalizeAviIndex(_frameCnt, false);

    static uint8_t idxOut[4096];
    size_t n = 0;
    do
    {
      n = writeAviIndex(idxOut, sizeof(idxOut), false);
      if (n)
        _file.write(idxOut, n);
    } while (n);

    // compute actual fps and patch header
    uint32_t durMs = millis() - _startMs;
    float actualFps = (durMs > 0) ? (1000.0f * _frameCnt) / durMs : _targetFps;
    uint8_t actualFpsInt = (uint8_t)lround(actualFps);

    buildAviHdr(actualFpsInt, _frameType, _frameCnt, false);

    // overwrite header at start
    _file.seek(0);
    _file.write(aviHeader, AVI_HEADER_LEN);
    _file.close();
    return true;
